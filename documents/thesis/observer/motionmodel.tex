...
\label{sec:observer:motionmodel}
    ...
    model's prediction.
    %~ In this section, a motion model is derived and evaluated.
...
%~ 2*qx*qz - 2*qy*qw    2*qy*qz + 2*qx*qw   1 - 2*qx2 - 2*qy2
        ...
        Rotation quaternions describing the rotation \textit{to frame \textbf{$a$} from frame \textbf{$b$}}
        are commonly denoted $q^{ab}$.
        %~ , whereas the \textit{b} may be dropped if the rotation is global, i.e. relative the NEDEF system.
    ...
    \subsection{Kinematics}
        ...
        and orientation, $q^{wb}$, by the relations in Eq.~\eqref{eq:observer:kinematicsc} \citep{Pounds_modellingand}. % more citations needed?
        \begin{subequations}
